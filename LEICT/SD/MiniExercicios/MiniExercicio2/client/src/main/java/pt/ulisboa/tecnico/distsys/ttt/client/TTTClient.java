		...
		try (Scanner scanner = new Scanner(System.in)) {
			...
			do {
				...
				do {
					...
					if(go == 10) {
						play_res = stub.aoCalhas(aoCalhasRequest.newBuilder().setPlayer(player).build()).getPlayRes();
						continue;
					}
					...
					play_res = stub.play(PlayRequest.newBuilder().setColumn(column).setRow(row).setPlayer(player).build()).getPlayResult();
