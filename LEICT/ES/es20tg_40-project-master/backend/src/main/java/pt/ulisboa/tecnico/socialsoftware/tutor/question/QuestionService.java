...
public class QuestionService {
    ...
    public QuestionDto findQuestionById(Integer questionId) {
        return questionRepository.findByKey(questionId).map(QuestionDto::new)
                .orElseThrow(() -> new TutorException(QUESTION_NOT_FOUND, questionId));
    }

    @Transactional(isolation = Isolation.REPEATABLE_READ)
    public CourseDto findQuestionCourse(Integer questionId) {
        return questionRepository.findByKey(questionId)
    ...
    public QuestionDto updateQuestion(Integer questionId, QuestionDto questionDto) {
        Question question = questionRepository.findByKey(questionId).orElseThrow(() -> new TutorException(QUESTION_NOT_FOUND, questionId));
    ...
    @Retryable(
            ...
            backoff = @Backoff(delay = 5000))
    @Transactional(isolation = Isolation.REPEATABLE_READ)
    public ArrayList<ClarificationRequest> consultClarificationRequests(Integer questionId) {
        Question question = questionRepository.findByKey(questionId).orElseThrow(() -> new TutorException(QUESTION_NOT_FOUND, questionId));
        return question.getClarificationRequests();
    }
